...
2. Call `run_tests` with `scope: "focused"`, a `behavior` label, literal worktree-relative `files`,
   and `testFullName`. Join describe names and the test name using the installed Vitest version's
   separator. Vitest 4 uses spaces (`outer inner works`); Vitest 5 uses `>` surrounded by spaces
   (`outer > inner > works`). Names are literal, not regexes. An array selects several names for one
   behavior. On no match, use the reported collected names and check the selected files. Do not
   restructure tests or broaden selection to make the tool run. Vitest 5's raw JSON `fullName` still
   uses spaces; Tau reconstructs native names from `ancestorTitles` and `title`.
