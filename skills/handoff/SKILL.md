...
1. Find the target. Run `herdr agent list` and keep the agents in the target workspace or whose
   ...
   With none, run `herdr pane list` and keep the panes in the target workspace or whose `cwd` is the
   target worktree. If exactly one is there, start Pi in it with
