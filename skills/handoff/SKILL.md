...
1. Find the target. Run `herdr agent list` and keep the agents in the target workspace or whose
   `cwd` is the target worktree. Drop agents named `worker-*` or `scout-*`: they are Tau subagents
   working for a parent. One match is the target; otherwise ask the user.
