...
describe('preparation ownership', () => {
  ...
  it.each(['', "; require('node:child_process').execFileSync('git', ['add', 'generated'])"])(
    'commits clean generated additions with the requested paths: %s',
    async (stage) => {
      const directory = await fixture(
      ...
      expect(JSON.stringify(result.content)).toContain('Preparation-added paths');
  ...
  it('classifies clean tracked generated changes separately from user edits', async () => {
    ...
    expect(result.details.groups[0]?.preparationAddedFiles).toEqual(['other']);
  ...
  it('includes generated paths from a nested cwd before checks review and one approval', async () => {
    const directory = await fixture(
    ...
    const previews: string[] = [];
    const approval = vi.spyOn(overlay, 'confirmCommitOverlay');
    ...
    expect(previews[0]).toContain('Preparation-added');
    expect(previews[0]).toContain('[preparation-added]');
    expect(previews[0]).not.toContain('Approve all remaining');
    expect(approval).toHaveBeenCalledTimes(1);
    expect(approval.mock.calls[0]?.[1]).toMatchObject({
      preparationAddedFiles: ['generated', 'other'],
      preparationDiff: await git(directory, ['diff', 'HEAD^', 'HEAD', '--', 'generated', 'other']),
    });
  ...
  it('reads the preparation diff for exactly the added literal paths', async () => {
    const directory = await fixture("require('node:fs').writeFileSync('generated*', 'prepared')");
    await writeFile(join(directory, 'generated-other'), 'requested bytes');
    const approval = vi
      .spyOn(overlay, 'confirmCommitOverlay')
      .mockImplementation(async (_context, view) => {
        expect(view.preparationAddedFiles).toEqual(['generated*']);
        expect(view.preparationDiff).toBe(
          await git(directory, [
            '--literal-pathspecs',
            'diff',
            '--cached',
        ...
        expect(view.preparationDiff).toContain('+prepared');
        expect(view.preparationDiff).not.toContain('generated-other');
        expect(view.preparationDiff).not.toContain('+requested bytes');

        return 'approve';
      });

    await executeCommit(directory, {
      groups: [{ files: ['requested', 'generated-other'], subject: 'feat: literal output' }],
    });

    expect(approval).toHaveBeenCalledTimes(1);
  });

  it.each(['working', 'private index', 'new addition'])(
    'rejects generated candidate changes around acceptance: %s',
    async (target) => {
      const directory = await fixture("require('node:fs').writeFileSync('generated', 'prepared')");
      const originalIndex = await readFile(join(directory, '.git/index'));
      const snapshotPreparation = preparation.snapshotPreparation;
      vi.spyOn(preparation, 'snapshotPreparation').mockImplementation(async (...arguments_) => {
        const ownership = await snapshotPreparation(...arguments_);
        const validate = ownership.validate;
        vi.spyOn(ownership, 'validate').mockImplementationOnce(async (...validationArguments) => {
          const candidate = await validate(...validationArguments);

          if (target === 'working') {
            await writeFile(join(directory, 'generated'), 'concurrent edit');
          } else if (target === 'private index') {
            await runCommand(
              'env',
              [
                `GIT_INDEX_FILE=${join(ownership.directory, 'candidate-index')}`,
                'git',
                'add',
                'generated',
              ],
            ...
            vi.spyOn(candidate, 'accept').mockImplementationOnce(async () => {
              await accept();
              await writeFile(join(directory, 'late-addition'), 'concurrent output');
            });
          }
      ...
      expect(await readFile(join(directory, '.git/index'))).toEqual(originalIndex);
      expect((await recovery(directory)).working).toHaveProperty('requested');
    },
  );
  ...
  it('reserves accepted generated paths against later preparation in the batch', async () => {
    ...
    const choices = ['approveAll', 'approve'];
    let preparations = 0;
    const reviewer = vi.fn<typeof reviewComments>(async () => ({ findings: [] }));
    const tool = createReviewedCommitTool(
      {
  ...
  it.each(['skip', 'abort', 'error'])(
    'retains preparation recovery when approval ends with %s',
    async (choice) => {
      const directory = await fixture("require('node:fs').writeFileSync('generated', 'prepared')");
      const originalIndex = await readFile(join(directory, '.git/index'));
      const reviewer = vi.fn<typeof reviewComments>(async () => ({ findings: [] }));
      ...
      const expected = {
        abort:
          /Commit declined by user[\s\S]*Preparation-added paths.*generated[\s\S]*Recovery saved/,
        skip: /Commit skipped by user[\s\S]*Recovery saved/,
  ...
  it.each(['working', 'staged', 'staged-only', 'tracked-index-only'])(
    'commits generated bytes from %s output after one approval',
    async (kind) => {
  ...
  it.each(['.env', '.ssh/id_rsa', ':generated'])(
    'rejects guarded generated path %s before approval',
    async (path) => {
      const directory = await fixture(
  ...
  it.each([true, false])(
    ...
    async (hasUI) => {
      ...
      const originalIndex = await readFile(join(directory, '.git/index'));
      const custom = vi.fn<() => Promise<string>>(() => Promise.resolve('approve'));
      const projectCheck = vi.spyOn(checker, 'runChecker');
      ...
      await expect(
        ...
        /Preparation added paths \(repository-relative\).*generated.*Inspect.*list them explicitly.*new commit call/,
      );
      expect(projectCheck).not.toHaveBeenCalled();
      expect(await git(directory, ['log', '-1', '--format=%s'])).toBe('test: baseline\n');
      expect(custom).not.toHaveBeenCalled();
      expect(reviewer).not.toHaveBeenCalled();
      expect(await readFile(join(directory, '.git/index'))).toEqual(originalIndex);
    },
