...
export const genericPrompt = (task: Task): string => {
  ...
  return [
    ...
    [`Task: ${task.taskId}`, task.task].join('\n'),
    ...
    [
      [
        `Publish your final plain text or Markdown report at ${path}.`,
        'This report area is already authorized; do not widen your sandbox.',
        'If you cannot write it, tell the manager and report the limit in text.',
        'No report means incomplete, even if you finish the task.',
      ].join(' '),
      [
        'Write the complete UTF-8 report to a new file named report.partial in that directory using exclusive creation.',
        'Close it, then publish report.md with a create-only hard link and remove report.partial.',
        'Never overwrite an existing file. Do not write report.md incrementally. Maximum size: 10000 bytes.',
        'Use this exact envelope, with your handoff between the headers and final line:',
      ].join(' '),
      `Task: ${task.taskId}`,
    ...
    [
      'The final End task line must end with a newline. Publish once.',
      'A saved receipt proves report delivery, not answer correctness.',
      'Native controls remain in force; Tau does not certify their enforcement.',
    ].join(' '),
  ].join('\n\n');
};
