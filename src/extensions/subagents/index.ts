...
const visibility = Type.Optional(
  StringEnum(['foreground', 'background'] as const, {
    description: [
      'Choose foreground (default) for visible work or background for separate worker tabs.',
      'Foreground shares parent/worker space, falling back to a separate tab when space is too small.',
...
const registerLaunchTool = (runtime: SubagentRuntime): void => {
  runtime.pi.registerTool({
    ...
    description: [
      ...
      'Profiles: investigator, worker. Pi (default) needs trusted-full-tools, CC Safety Net, explicit/configured provider/id model.',
      'Other harnesses need native-controls and writable cwd/.tau/workers/<taskId>/report.md.',
      'Literal nativeArguments default empty; model requests need native flags. Tau neither verifies native models/controls nor approves dialogs.',
      'Assign acceptance criteria, baseline, worktree, one editor per worktree. Reports: Changes, Evidence, Decisions, Concerns.',
      'Workers cannot launch workers; ask the parent. Per-parent live cap; deadline includes waits/cleanup.',
      ...
      'Ask/report/stop snapshots start a parent turn after the current tool call. End your turn to wait; never sleep or poll.',
...
const registerHistoryTool = (runtime: SubagentRuntime): void => {
  runtime.pi.registerTool({
    ...
    description: [
      'Search earlier work in this session history by name, task/native session ID, or description. No required inputs; query, offset, and limit are optional.',
      'Returns candidates and totalMatches, excluding current/ancestor sessions. Use nextOffset with the same query to page; additions can shift pages.',
      'truncatedFields marks previews, not exact IDs/paths; reportFile locates a truncated report. Clarify multiple matches using full IDs.',
...
const registerStatusTool = (runtime: SubagentRuntime): void => {
  runtime.pi.registerTool({
    ...
    label: 'Worker status',
    description: [
      'Read a direct child task. Requires taskId; questionId selects Pi reply/acknowledgement, submissionId selects native delivery, readOutput reads active native terminal text.',
...
const registerReplyTool = (runtime: SubagentRuntime): void => {
  runtime.pi.registerTool({
    ...
    description: [
      ...
      'Inspect status with questionId for Pi or submissionId for other harnesses. Tau refuses blocked/unknown native state and never approves dialogs.',
    ].join(' '),
    parameters: replyParameters,
    renderCall(parameters, theme) {
